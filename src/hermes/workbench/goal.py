...
# ---------------------------------------------------------------------------
# Unified task status constants (shared with task-tracker skill)
# ---------------------------------------------------------------------------
...
def is_terminal_status(status: str) -> bool:
    """Return True if *status* is a terminal task state."""
...
class PlannerAgent(SubAgent):
    ...
    def _plan_via_llm(
        ...
        summaries = self.runner.discover_summaries()
        available = "\n".join(
            f"- {s.name}: {s.description}" for s in summaries
        ) or "(none)"
        sys_prompt = (
            "You are a planning agent. Given a goal and a list of available "
            "skills (with descriptions), produce a JSON array of plan steps. "
            "Each step MUST be an object with: \"skill\" (string, one of the "
            "available skills), \"args\" (array of strings, may be empty), and "
            "optionally \"timeout\" (number, seconds) and \"abort_on_error\" "
            "(boolean). Use the descriptions to pick the most relevant skills."
        )
        user_prompt = (
